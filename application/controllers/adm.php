...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '".bersih($p,"nama")."', nis = '".bersih($p,"nis")."', alamat = '".bersih($p,"alamat")."' , 
				tmpt_lahir = '".bersih($p,"tmpt_lahir")."' , tgl_lahir = '".bersih($p,"tgl_lahir")."' , no_tlp = '".bersih($p,"no_tlp")."'	WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_siswa VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"nis")."', '".bersih($p,"alamat")."', '".bersih($p,"tmpt_lahir")."', '".bersih($p,"tgl_lahir")."', '".bersih($p,"no_tlp")."')");
			}
	...
	public function m_guru() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_guru SET nama = '".bersih($p,"nama")."', nip = '".bersih($p,"nip")."', pendidikan = '".bersih($p,"pendidikan")."', alamat = '".bersih($p,"alamat")."',
				no_tlp = '".bersih($p,"no_tlp")."' WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
				
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_guru VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"nip")."', '".bersih($p,"pendidikan")."', '".bersih($p,"alamat")."', '".bersih($p,"no_tlp")."')");
			}
...
public function upload_file(){
		...
		$file = $this->input->post('file_materi');
		 $targetfolder = "./upload/materi_pelajaran/";
        $targetfolder = $targetfolder . basename( $_FILES['file_materi']['name']) ;
        ...
        if(move_uploaded_file($_FILES['file_materi']['tmp_name'], $targetfolder))
 {
        echo "File ". basename( $_FILES['file_materi']['name'])." Berhasil di-Upload";
 //Jalankan perintah insert ke database
$this->db->query("INSERT INTO m_materi VALUES ('','".$_POST['id_guru']."','".$_POST['id_mapel']."','".$_POST['nama_materi']."', '".basename( $_FILES['file_materi']['name'])."', '".date('Y-m-d H:i:s')."')");
			redirect('adm/m_materi');
 }
 else {
 ...
 echo "Hanya Boleh upload PDF, JPG, JPEG dan PNG .<br>";
}
}
    
	public function m_materi1() {
		$this->cek_aktif();
		...
		$a['data'] = $this->db->query("SELECT m_materi.*,
                                        m_guru.nama AS id_guru,
                                        m_mapel.nama AS id_mapel
                                        FROM m_materi
                                        INNER JOIN m_guru
                                        ON m_materi.id_guru = m_guru.id
                                        INNER JOIN m_mapel
                                        ON m_materi.id_mapel = m_mapel.id
										WHERE m_materi.id
										ORDER BY m_materi.id ASC")->result();
    ...
    public function m_materi() {
		...
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_materi WHERE id = '$uri4'")->row();
			$this->j($a);
			exit();
		} else if ($uri3 == "hapus_gambar") {
			$nama_gambar = $this->db->query("SELECT gambar FROM m_materi WHERE id = '".$uri5."'")->row();
			$this->db->query("UPDATE m_materi SET gambar = '' WHERE id = '".$uri5."'");
			@unlink("./upload/gambar_materi/".$nama_gambar->gambar);
			...
			$status = "";
			$msg = "";
			$file_element_name = 'gambar_materi';

			//echo var_dump($_POST);

			$pembuat_materi = ($a['sess_level'] == "guru") ? $_POST['id_guru'] : $a['sess_konid'];
			$pembuat_materi_u = ($a['sess_level'] == "guru") ? ", id_guru = '".$_POST['id_guru']."'" : "";
			...
			if ($status != "error") {
				...
				if (!$this->upload->do_upload($file_element_name)) {
					...
					$ambil_gambar = $this->db->query("SELECT file_materi FROM m_materi WHERE id = '".$_POST['id']."'")->row();
					if ($_POST['id'] != "") {
						$this->db->query("UPDATE m_materi SET id_mapel = '".$_POST['id_mapel']."', 
										file_materi = '".$data['file_name']."' 
	...
	public function m_soal() {
		...
		if ($uri3 == "det") {
			...
			$html = "<link href='".base_url()."___/css/style_print.css' rel='stylesheet' media='' type='text/css'/><head><meta charset='utf-8'><title>Aplikasi Bimbingan Belajar Primagama</title><meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no'><meta name='apple-mobile-web-app-capable' content='yes'><link rel='shortcut icon' href='".base_url()."___/img/logo.png'></head><div class='ctr'><img src='".base_url()."___/img/logo_l.png' width='200px'></div>";
			if ($a['sess_level'] == "admin") {
				$data = $this->db->query("SELECT * FROM m_soal WHERE id_mapel = '$uri4'")->result();
			} else {
				$data = $this->db->query("SELECT * FROM m_soal WHERE id_guru = '".$a['sess_konid']."' AND id_mapel = '$uri4'")->result();
			...
			if (!empty($data)) {
				$html .= "
                <h3>Mata Pelajaran: ".$mapel->nama."</h3>
                <hr style='border: solid 1px #000'>";
				$no = 1;
				$jawaban = array("A","B","C","D","E");
				foreach ($data as $d) {
					if ($d->gambar == "") {
                        $html .= "<h4>Soal ".$no.": ".$mapel->nama."</h4>";
		                $html .= 
                        '<table>
		                ...
		                for ($j=0; $j<sizeof($jawaban);$j++) {
		                  ...
		                  if ($jawaban[$j] == $d->jawaban) {
		                    $html .= '<tr>
                            <td width="2%">'.$jawaban[$j].'</td>
                            <td><label for="opsi_'.$jawaban[$j].'_'.$d->id.'">'.$opsyens.'</label></td></label></tr>';
		                  } else {
		                    $html .= '<tr>
                            <td width="2%">'.$jawaban[$j].'</td>
                            <td><label for="opsi_'.$jawaban[$j].'_'.$d->id.'">'.$opsyens.'</label></td></label></tr>';
		                ...
		                for ($j=0; $j<sizeof($jawaban);$j++) {
		                  ...
		                  if ($jawaban[$j] == $d->jawaban) {
		                    $html .= '<tr><td width="2%">'.$jawaban[$j].'</td>
                            <td><label for="opsi_'.$jawaban[$j].'_'.$d->id.'">'.$opsyens.'</label></td></label></tr>';
		                  } else {
		                    $html .= '<tr><td width="2%">'.$jawaban[$j].'</td>
                            <td><label for="opsi_'.$jawaban[$j].'_'.$d->id.'">'.$opsyens.'</label></td></label></tr>';
				...
				$html .= "Belum Ada Data<br>";
			}
            $html .= '<br><div class="ctr">&copy; 2019. Aplikasi Bimbingan Belajar Primagama.</div>';
	...
	public function act_login() {
		...
		if ($j_data === 1) {
			...
			$_log['log']['status']			= "1";
			$_log['log']['keterangan']		= "Masuk Berhasil";
			$_log['log']['detil_admin']		= $this->session->userdata;
		} else {
			$_log['log']['status']			= "0";
			$_log['log']['keterangan']		= "Maaf, username dan password salah/tidak ditemukan";
